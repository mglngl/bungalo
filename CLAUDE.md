...
- **Roles:** `jugador` (juega y consulta) y `admin`. El rol se evalúa **por torneo**: quien **crea** un torneo es su **admin** (gestiona jornadas/partidos, **formato y reglamento de puntuación —§5—** y participantes de **ese** torneo); en torneos a los que solo se unió, es `jugador`.
...
Cada `Partido` pertenece a una **fase** (`Partidos.fase`), que el sistema normaliza desde el proveedor de API (§6) o asigna el `admin`:
...
El **formato** es una propiedad del torneo (`Quinielas.formato ∈ {casual, incremental, personalizado}`), elegida por el `admin` al crearlo y **congelada al iniciar el primer partido** (ver §5.5).
...
El `admin` define **individualmente** la cantidad exacta de puntos que se otorgan por **resultado correcto** y por **marcador exacto** para **cada una de las fases** del torneo. No usa el multiplicador implícito del formato Incremental (el escalamiento se logra con los propios valores por fase).

- Por cada `fase`, el `admin` fija `P_RES(fase)` y `P_EXACTO(fase)` (enteros ≥ 0).
...
- **`Quinielas`** — `id`, `nombre`, `temporada`, `codigo` (6 caracteres alfanuméricos, **único**, para invitar/unirse), `visibilidad` (**siempre `privada`**: el torneo solo admite ingreso por invitación con `codigo`; no existe valor `publica`), `id_usuario_creador`, `formato` (`casual`|`incremental`|`personalizado`; mecánica de puntuación del torneo, §5), `estado` (`activa`|`cerrada`), `id_reglas_puntuacion` (conjunto de reglas efectivas por fase del torneo, §5.4), `reglas_congeladas` (bool; `true` cuando inicia el primer partido del torneo y la puntuación queda inmutable, §5.5), `creado_en`
...
- `crearQuiniela(nombre, temporada, formato, reglas?)` — crea un torneo con su **formato** (`casual`|`incremental`|`personalizado`, §5); para `personalizado`, `reglas` trae los puntos por **resultado** y **marcador exacto** **por fase** (§5.3.3), que se validan y **congelan al lanzamiento** (§5.5). El **servidor genera el `codigo` único de 6 caracteres alfanuméricos** (con verificación de colisión) y registra al creador en `Participaciones` con `rol_en_quiniela=admin`. Devuelve el código.
...
│
├── LICENSE                   # licencia restrictiva — Todos los derechos reservados (sección 14)
├── package.json
...
│   └── robots.txt            # Disallow a crawlers no autorizados + noai + noindex (sección 14)
└── apps-script/
...
`vite.config.js` debe fijar `base` al nombre del repo **y producir un único `index.html` autocontenido** (inline total, sin sourcemaps, minificado y sin comentarios — ver sección 14):
...
export default defineConfig({
  ...
  build: {
    target: 'esnext',
    sourcemap: false,                    // PI: SIN source maps en producción (§14)
...
## 14. Protección de propiedad intelectual (PI) y disuasión legal
